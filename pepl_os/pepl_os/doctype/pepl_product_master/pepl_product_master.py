...
class PEPLProductMaster(Document):
    ...
    def validate(self):
        ...
        if self.product_type == "Single Component" and self.assembly_components:
            frappe.throw(
                _(
                    "Single Component products cannot have Assembly Components. "
                    "Change Product Type to Assembly or Sub-Assembly, or remove components."
                )
            )

    def _create_linked_item(self):
        """Auto-create an ERPNext Item if not linked."""
        existing = frappe.db.get_value("Item", {"item_name": self.product_name}, "name")
        if existing:
        ...
        try:
            ...
            if self.product_type in ["Assembly", "Sub-Assembly"]:
                item.is_sub_contracted_item = 0
            item.insert(ignore_permissions=True)
            self.linked_item = item.name

            frappe.msgprint(
                _("Auto-created ERPNext Item: {0}").format(item.name),
                indicator="green",
                alert=True,
            )
        except Exception as e:
            frappe.throw(_("Failed to auto-create Item: {0}").format(str(e)))
...
def get_product_for_item(item):
    ...
    product = frappe.db.get_value(
        ...
        [
            ...
            "drawing_number",
            "pl_number",
            "current_drawing_revision",
            "current_drawing_file",
            "sector",
            "sub_sector",
            "product_type",
            "primary_customer",
...
def sync_components_from_bom(product_name):
    ...
    if not product.linked_bom:
        frappe.throw(
            _("No BOM linked. Set Linked BOM first or ensure linked Item has an active default BOM.")
        )
